...
export default function SignupScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.replace('/'); // Go to home after successful sign‑up
    } catch (e: any) {
      setErrorMsg(e.message || 'Erreur lors de l’inscription');
      triggerShake();
    } finally {
      setLoading(false);
    }
    ...
    <Animated.View style={[styles.container, { opacity: fadeAnim, transform: [{ translateX: shakeAnim }] }]}>
      ...
      <View style={styles.formWrapper}>
        ...
        <TextInput
          ...
          style={styles.input}
          mode="outlined"
          left={<TextInput.Icon icon={() => <MaterialCommunityIcons name="lock-check" size={20} />} />}
        ...
        >
          {loading ? <ActivityIndicator animating={true} color="white" /> : 'Créer mon compte'}
        </Button>

        <View style={styles.linksRow}>
          <Text style={styles.link} onPress={() => router.push('/auth')}>Déjà un compte ? Se connecter</Text>
        </View>
      </View>
    </Animated.View>
  );
