...
export default function LoginScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.replace('/catalogue'); // redirect to catalogue after login
    } catch (e: any) {
      setErrorMsg(e.message || 'Identifiants incorrects');
      triggerShake();
    } finally {
      setLoading(false);
    }
    ...
    <Animated.View style={[styles.container, { opacity: fadeAnim, transform: [{ translateX: shakeAnim }] }]}>
      ...
      <View style={styles.formWrapper}>
        ...
        <TextInput
          ...
          right={<TextInput.Icon icon={() => <MaterialCommunityIcons name={showPassword ? 'eye-off' : 'eye'} size={20} />} onPress={() => setShowPassword(!showPassword)} />}
        ...
        >
          {loading ? <ActivityIndicator animating color="white" /> : 'Se connecter'}
        </Button>

        <View style={styles.linksRow}>
          <Text style={styles.link} onPress={() => router.push('/forgot')}>Mot de passe oublié ?</Text>
          <Text style={styles.link} onPress={() => router.push('/signup')}>Créer un compte</Text>
        </View>
      </View>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  ...
  errorBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFEBEE', padding: 8, borderRadius: 4, marginBottom: 8 },
  errorText: { color: '#fff', marginLeft: 8, fontSize: 14 },
});
